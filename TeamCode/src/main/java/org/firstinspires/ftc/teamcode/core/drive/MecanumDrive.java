 ...
 *
 * Quy ước trục field: +X sang phải, +Y tiến; heading giảm theo chiều ngược kim đồng hồ.
 * Frame robot: +forward tiến, +strafe trái, +rotate CCW.
 *
 * Gọi {@code localizer.update()} trước {@link #update()}.
 */
public final class MecanumDrive {
    ...
    /**
     * OFF — hành vi cũ: tắt rotate khi HOLDING (phù hợp lái thủ công).
     * ALWAYS — giữ heading bằng PID cả khi đã tới đích; dùng khi vẽ (tránh trôi heading khi đứng yên).
     */
    public enum HeadingHoldMode { OFF, ALWAYS }
    ...
    private HeadingHoldMode headingHoldMode = HeadingHoldMode.OFF;
    ...
    /** Không motor — chỉ mô phỏng offline. */
    ...
    public HeadingHoldMode getHeadingHoldMode() { return headingHoldMode; }

    public void setHeadingHoldMode(HeadingHoldMode mode) {
        if (mode == null) throw new IllegalArgumentException("mode");
        headingHoldMode = mode;
    }
    ...
    private void updatePositionControl() {
        ...
        // Chế độ OFF: tắt rotate khi đủ gần hoặc khi HOLDING (hành vi lái thông thường).
        // Chế độ ALWAYS: luôn chạy PID heading — robot vẽ cần giữ góc khi đứng yên để không trôi nét.
        if (headingHoldMode != HeadingHoldMode.ALWAYS) {
