...
public final class LiftingHardwareManagerTest {
    ...
    public static void main(String[] args){
        ...
        ForkServoManager fork=new ForkServoManager((v)->{},(v)->{},.2,.8,.5,.5); fork.setPose(ForkServoManager.Pose.HOLD); check("fork hold",fork.pose()==ForkServoManager.Pose.HOLD); check("invalid pose",!new HardwareContracts.PoseReading(Double.NaN,1,0,1).valid); Clock c=new Clock(); CameraAdapterManager cameras=new CameraAdapterManager(new Pi5UartCameraTransport(c),100); check("UART webcam1 blocked",!cameras.movementAuthorized(CameraChannel.WEBCAM1,0)); check("UART webcam2 blocked",!cameras.movementAuthorized(CameraChannel.WEBCAM2,0)); ReleaseBackoutSensorManager release=new ReleaseBackoutSensorManager(left::active,()->new HardwareContracts.PoseReading(1,2,3,4)); check("release reading",release.released()&&release.reading().valid);
