...
/**
 * Test offline cho bộ lọc temporal cúa SingleTargetCamera.
 * Chạy: java SingleTargetCameraTest (không cần phần cứng FTC).
 *
 * Bô lọc hoàn toàn giống SingleTargetPipeline, không có label
 * hysteresis hay multi-target logic. Kiểm tra 6 scenario:
 *
 *   1. Jitter reduction     — noise +-4px → output <= 1.5px
 ...
 *
 * Note: detectSingleTemplate, perspectiveTransform, ORB không
 * duọc test o dây vì can OpenCV. Test này chi kiêm tra bô lọc
 * temporal và state machine.
 */
public final class SingleTargetCameraTest {

    // -------------- Hằng số filter (copy từ SingleTargetCamera) --------------
    ...
    // ============================================================
    //  Mini filter (copy từ SingleTargetPipeline)
    // ============================================================
    ...
    /**
     * Feed a raw detection into the filter.
     * label=null means miss (no detection in frame).
     */
    ...
    // ============================================================
    //  Filter methods (copy từ SingleTargetPipeline)
    // ============================================================
    ...
    private void updatePositionFilter(double newDx, double newDy, double confidence) {
        ...
        double effectiveDeadband = CENTER_DEADBAND_PX * (1.0 + 1.0 - confidence);
    ...
    static void testJitterReduction() {
        System.out.println("\n--- Test 1: Jitter reduction ---");
        SingleTargetCameraTest f = new SingleTargetCameraTest();
    ...
    static void testSpikeRejection() {
        ...
        // Spike frames 1-3: outlier held by missHold logic
        ...
        // Frame 4 (outlierStreak==4) confirms spike, resets filter
        ...
        // Recovery: 4 outlier frames back to 52 (52 is outlier vs 250)
    ...
    static void testSetTargetReset() {
        ...
        // Detect target1, build up filter state
        for (int i = 0; i < 10; i++) {
        ...
        // Switch to target_b (simulates setTarget)
    ...
    public static void main(String[] args) {
        System.out.println("SingleTargetCamera filter tests");
        System.out.println("================================");
